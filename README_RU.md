...
### 3. Ошибки импорта смежных модулей
* **Проблема:** Многие кастомные ноды ломаются, если папка проекта переименована (например, из `ComfyUI-LTXVideo` в `ComfyUI-LTXVideo-master`) или установлены конфликтующие форки (например, AVSplit).
* **Решение:** Встроенный **ультра-надежный резолвер модулей** динамически сканирует папку `custom_nodes`, отдает приоритет точным совпадениям, игнорирует альтернативные библиотеки и безопасно подгружает модули «на лету».
...
> [!NOTE]
> *Сюда вы можете вставить скриншот ноды `🅛🅣🅧 MSR IC LORA FLF (Standalone)` из вашего интерфейса ComfyUI.*
...
| **msr_width / msr_height** | INT | `736 / 1280` | Разрешение, до которого будут сжаты все MSR-кадры перед кодированием VAE. |
| **msr_frame_count** | COMBO | `17` | Количество кадров в сетке MSR (17, 25, 33, 41). Больше = стабильнее лицо, но больше VRAM. |
...
| **msr_strength** | FLOAT | `1.0` | Сила прямой инжекции MSR-латентов в пространство генерации. |
...
| **lock_first_frame** | BOOLEAN | `False` | Включает жесткую Inplace-заморозку пикселей первого кадра. |
| **lock_first_frame_len** | INT | `2` | Количество замороженных латентных кадров (устраняет размытие от интерполяции VAE). |
| **first_frame_strength** | FLOAT | `1.0` | Сила прямой инжекции латентов первого кадра. |
| **first_frame_attn_strength**| FLOAT | `1.0` | Сила влияния перекрестного внимания первого кадра. |
| **last_frame** | IMAGE | *Опционально* | Изображение для самого последнего кадра видео. |
| **lock_last_frame** | BOOLEAN | `False` | Включает жесткую Inplace-заморозку пикселей последнего кадра. |
| **last_frame_strength** | FLOAT | `1.0` | Сила прямой инжекции латентов последнего кадра. |
| **last_frame_attn_strength**| FLOAT | `1.0` | Сила влияния перекрестного внимания последнего кадра. |
| **latent_downscale_factor**| FLOAT | `1.0` | Коэффициент сжатия латентов (например, 2.0 для сжатых моделей IC-LoRA). |
| **crop** | COMBO | `disabled` | Метод изменения размера (`disabled` растягивает, `center` обрезает пропорционально). |
...
2. Склонируйте этот репозиторий:
   ...
   ```
3. Перезапустите ComfyUI. Нода появится в категории:
...
1. Загрузите желаемый **Latent** (например, пустой латент 736x1280, 97 кадров).
2. Подключите **Positive** и **Negative** кондишены от ваших текстовых промптов.
3. Подключите вашу модель **VAE** во вход `vae`.
4. Настройте ваши референсные материалы:
   * Загрузите стартовое изображение в `first_frame`. Включите `lock_first_frame` (задайте длину `2` для идеальной четкости).
   * Загрузите до 4 изображений персонажа с разных ракурсов в слоты `msr_image_1` - `4` для удержания лица.
   * Загрузите конечное изображение в `last_frame` и включите `lock_last_frame` для бесшовной зацикленности.
5. Направьте выходы ноды (`positive`, `negative`, `latent`) напрямую во вход вашего сэмплера (KSampler) или гида (CFGGuider).
